...
Add `swift-google-cloud-assuredworkloads-v1` as a package dependency:

```bash
swift package add-dependency https://github.com/googleapis/swift-google-cloud-assuredworkloads-v1.git --from 0.1.0-preview
```

Then add `GoogleCloudAssuredWorkloadsV1` to your target's dependencies:

```bash
swift package add-target-dependency GoogleCloudAssuredWorkloadsV1 <target-name> --package swift-google-cloud-assuredworkloads-v1
```
